...
$result = Pay::wechat($config)->refund($order); // 返回 `XiaoYun\Supports\Collection` 实例，可以通过 `$result->xxx` 访问服务器返回的数据。


// 查询
$result = Pay::wechat($config)->find('out_trade_no_123456'); // 返回 `XiaoYun\Supports\Collection` 实例，可以通过 `$result->xxx` 访问服务器返回的数据。
...
// 关闭
$result = Pay::wechat($config)->close('out_trade_no_123456'); // 返回 `XiaoYun\Supports\Collection` 实例，可以通过 `$result->xxx` 访问服务器返回的数据。
...
// 是的，验签就这么简单！
$data = $wechat->verify(); // 返回 `XiaoYun\Supports\Collection` 实例，可以通过 `$data->xxx` 访问服务器返回的数据。
